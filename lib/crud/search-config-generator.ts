...
export function enhanceConfigWithSearchFields(config: EntityConfig): EntityConfig {
  ...
  // filterFieldsManualOnly: 仅用手写字段，不合并自动生成（如提柜管理）
  // 否则：用手动定义的字段覆盖自动生成的对应字段，保留其他自动生成的字段
  const autoGeneratedFields = generateFilterFields(enhancedConfig)
  if (enhancedConfig.list.filterFieldsManualOnly) {
    enhancedConfig.list.filterFields = enhancedConfig.list.filterFields ?? []
  } else if (!enhancedConfig.list.filterFields || enhancedConfig.list.filterFields.length === 0) {
  ...
  // 可通过 advancedSearchEnabled: false 关闭（如提柜管理）
  if (enhancedConfig.list.advancedSearchEnabled === false) {
    enhancedConfig.list.advancedSearchFields = []
  } else {
